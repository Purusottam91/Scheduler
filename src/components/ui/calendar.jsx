  ...
  return (
    <DayPicker
      ...
      classNames={{
        ...
        head_row: "grid grid-cols-7 gap-0",
        head_cell:
          "text-black rounded-md w-9 h-9 flex items-center justify-center text-[0.8rem] font-normal bg-white uppercase",
        row: "grid grid-cols-7 gap-0 mt-2",
        cell: "relative p-0 text-center text-sm focus-within:relative focus-within:z-20 [&:has([aria-selected])]:bg-accent [&:has([aria-selected].day-outside)]:bg-accent/50 [&:has([aria-selected].day-range-end)]:rounded-r-md first:[&:has([aria-selected])]:rounded-l-md last:[&:has([aria-selected])]:rounded-r-md",
        day: cn(
          buttonVariants({ variant: "ghost" }),
          "h-9 w-9 p-0 font-normal aria-selected:opacity-100 bg-white text-black"
        ),
        day_range_end: "day-range-end",
        day_selected:
          "bg-primary text-primary-foreground hover:bg-primary hover:text-primary-foreground focus:bg-primary focus:text-primary-foreground",
        day_today: "bg-accent text-accent-foreground",
        day_outside:
          "day-outside text-black opacity-50 aria-selected:bg-accent/50 aria-selected:text-black aria-selected:opacity-30",
        ...
        HeadCell: ({ date, locale, ...props }) => {
          ...
          return (
            <div
              {...props}
              className={cn(
                "text-black rounded-md w-10 h-10 flex items-center justify-center text-sm font-semibold bg-gray-100 select-none tracking-wide uppercase"
              )}
            >
              {weekday}
            </div>
